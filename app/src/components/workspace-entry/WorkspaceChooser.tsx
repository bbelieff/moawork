...
export function WorkspaceChooserView({
  ...
  return <main className={styles.page}><section className={`${styles.shell} ${styles.compactShell}`} aria-labelledby="workspace-chooser-title"><header className={styles.protoTop}><Logo height={28} href="/" /></header><div className={styles.compactHub}><div className={styles.chooserHeader}><h1 id="workspace-chooser-title" className={styles.chooserTitle}>회사 선택</h1><Link className={styles.chooserAdd} href="/workspace-entry?mode=new"><span aria-hidden="true">+</span>회사 추가</Link></div><WorkspaceSelectionNotice notice={view.notice} /><ul className={styles.chooserList}>{workspaces.map((workspace) => <li key={workspace.orgId}><button type="button" onClick={() => onSelect(workspace.orgId)} disabled={view.locked}><span className={styles.workspaceMark} aria-hidden="true">{workspace.name.slice(0, 1)}</span><span><strong>{workspace.name}</strong>{(nameCounts.get(workspace.name) ?? 0) > 1 ? <small>{workspace.slug}</small> : null}</span>{badges?.[workspace.orgId] ? <Badge state={badges[workspace.orgId]!} label={workspace.name} /> : null}<b aria-hidden="true">→</b></button></li>)}</ul></div></section></main>;
}
