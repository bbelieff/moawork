...
/**
 * #547 — 어느 그룹에도 안 걸린 업무를 담는 «미분류» 묶음.
 *
 * 예전에는 이런 업무가 있으면 화면이 통째로 죽었고(검증기가 스냅샷 전체를 거부),
 * 그걸 고친 뒤에도 그냥 두면 이번엔 «조용히 안 보이는» 상태가 된다 —
 * 그룹을 돌며 `item.groupId === group.id` 로만 그리기 때문이다.
 * 죽지도 숨지도 않게, 여기에 모아서 보여 준다. 그래야 사용자가 그것을 옮기거나 지울 수 있다.
 */
...
function TableView({ snapshot, items, onOpen }: { snapshot: WorkBoardSnapshot; items: WorkItemSnapshot[]; onOpen: (item: WorkItemSnapshot) => void }) {
  ...
  const ungrouped = items.filter((item) => !item.groupId || !groupIds.has(item.groupId));
  const buckets = [
    ...snapshot.groups.map((group) => ({ id: group.id, name: group.name, color: group.color, real: true })),
    ...(ungrouped.length ? [{ id: UNGROUPED, name: "미분류", color: "#E7E5E0", real: false }] : []),
  ];
  return renderTable({ snapshot, items, onOpen, buckets, ungrouped });
}

function renderTable({ snapshot, items, onOpen, buckets, ungrouped }: {
  snapshot: WorkBoardSnapshot; items: WorkItemSnapshot[]; onOpen: (item: WorkItemSnapshot) => void;
  buckets: { id: string; name: string; color: string; real: boolean }[]; ungrouped: WorkItemSnapshot[];
}) {
  return <div className={styles.tableWrap}><table><thead><tr>{snapshot.columns.map((column, index) => <th key={column.key} className={column.key === "title" ? styles.sticky : undefined}>{column.label}{canManageStructure(snapshot.role) && !column.system ? <details><summary aria-label={`${column.label} 열 메뉴`}>...</summary><ActionForm snapshot={snapshot} operation="rename_column"><input type="hidden" name="columnKey" value={column.key}/><input name="value" defaultValue={column.label}/><button>이름 변경</button></ActionForm><ActionForm snapshot={snapshot} operation="reorder_column"><input type="hidden" name="columnKey" value={column.key}/><input type="number" min="0" name="position" defaultValue={index}/><button>이동</button></ActionForm><ActionForm snapshot={snapshot} operation="delete_column"><input type="hidden" name="columnKey" value={column.key}/><button>삭제</button></ActionForm></details> : null}</th>)}</tr></thead><tbody>{buckets.flatMap((group, groupIndex) => {
    const rows = group.real ? items.filter((item) => item.groupId === group.id) : ungrouped;
    return [<tr key={`g-${group.id}`} className={styles.groupRow}><th colSpan={snapshot.columns.length}><span style={{ background: group.color }}/>{group.name} <small>{rows.length}</small>{group.real && canManageStructure(snapshot.role) ? <details><summary aria-label={`${group.name} 그룹 메뉴`}>...</summary><ActionForm snapshot={snapshot} operation="rename_group"><input type="hidden" name="groupId" value={group.id}/><input name="value" defaultValue={group.name}/><button>이름 변경</button></ActionForm><ActionForm snapshot={snapshot} operation="reorder_group"><input type="hidden" name="groupId" value={group.id}/><input type="number" min="0" name="position" defaultValue={groupIndex}/><button>이동</button></ActionForm><ActionForm snapshot={snapshot} operation="delete_group"><input type="hidden" name="groupId" value={group.id}/><button>삭제</button></ActionForm></details> : null}</th></tr>, ...rows.map((item) => <tr key={item.id}>{snapshot.columns.map((column) => <td key={column.key} className={column.key === "title" ? styles.sticky : undefined}>{column.key === "title" ? <button className={styles.detailTrigger} onClick={() => onOpen(item)} aria-label={`${item.title} 상세 열기`}>{item.title}</button> : <Cell snapshot={snapshot} item={item} column={column}/>}</td>)}</tr>)];
  })}</tbody></table></div>;
}
...
function Detail({ snapshot, item, onClose }: { snapshot: WorkBoardSnapshot; item: WorkItemSnapshot; onClose: () => void }) {
  ...
  return <aside className={`${styles.drawer} mw-layer-dialog`} role="dialog" aria-modal="true" aria-label={`${item.title} 상세`} onKeyDown={(event) => { if (event.key === "Escape") onClose(); }}><header><div><small>업무 상세</small><h2>{item.title}</h2></div><button autoFocus onClick={onClose} aria-label="상세 닫기">×</button></header><nav>{(["updates", "files", "activity"] as const).map((key) => <button key={key} aria-current={tab === key ? "page" : undefined} onClick={() => setTab(key)}>{key === "updates" ? "업데이트" : key === "files" ? "파일" : "활동 로그"}</button>)}</nav>
    ...
    {snapshot.role !== "viewer" ? <div className={styles.detailBody}><ActionForm snapshot={snapshot} item={item} operation="move_item"><select name="groupId" defaultValue={item.groupId ?? ""} aria-label="이동할 그룹">{/* #547 — 그룹 없는 업무는 여기서 처음으로 그룹을 고르게 된다. 빈 값을 기본으로 둔다. */}{item.groupId ? null : <option value="">미분류</option>}{snapshot.groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select><button>업무 이동</button></ActionForm>{canManageStructure(snapshot.role) ? <ActionForm snapshot={snapshot} item={item} operation="delete_item"><button>업무 삭제</button></ActionForm> : null}</div> : null}
  </aside>;
}
