...
export function BuilderWorkspaceSurface({ snapshot }: Readonly<{ snapshot: WorkspaceOpsSnapshot }>) {
  ...
  const save = async () => setNotice((await saveBuilder({ tabs: label ? [{ id: crypto.randomUUID(), label, boardId }] : [] }, crypto.randomUUID())).message);
  const dryRun = async () => { const id = crypto.randomUUID(); const result = await createCsvDryRun(id, boardId, rows, crypto.randomUUID()); if (result.ok) setBatchId(id); setNotice(result.message); };
