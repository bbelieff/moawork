...
export function NewLeadIntakeForm({ boardId, groupId }: { boardId: string; groupId: string }) {
  ...
  useEffect(() => {
    if (state.ok && detailsRef.current) { formRef.current?.reset(); detailsRef.current.open = false; summaryRef.current?.focus(); }
  }, [state]);
  ...
  return (
    <><details ref={detailsRef} onToggle={(event) => setOpen(event.currentTarget.open)} className="group/newlead">
      ...
      <form ref={formRef} action={action} noValidate className="mt-2 grid w-[min(34rem,calc(100vw-2rem))] gap-3 rounded-xl border border-mw-line bg-mw-card p-4 shadow-lg">
