...
import { DashboardEvidenceList, DashboardFilterBar, DashboardStateNotice, MetricTabs, TodayTaskList, type AssigneeOption } from "@/components/dash/drilldown";
...
export default async function DashboardTasksPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [visibleDeals, pipelines] = await Promise.all([service.listDeals(ctx), service.listPipelines(ctx)]);
