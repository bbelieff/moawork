...
/**
 * 비대면 상담(STEP2) 탭의 제품 진입점.
 *
 * 같은 리드컨택 정본 보드의 단계 보기다 — 보드·회사·딜·아이템을 복제하지 않고
 * `?consultation=remote` 로 같은 보드에 닿는다. 기존 `/contract` URL·동작은
 * 그대로 둔다(값 없음 = 전체 보기).
 */
export default async function ConsultRemotePage({
  searchParams,
}: {
  ...
  if (result.kind !== "ready") {
    ...
    return (
      <section className="rounded-md border border-mw-line bg-mw-card p-5" aria-labelledby="consult-remote-entry-title">
        <h1 id="consult-remote-entry-title" className="text-lg font-semibold text-mw-fg">
          {conflict
