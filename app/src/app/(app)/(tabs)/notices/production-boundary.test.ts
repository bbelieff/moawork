...
describe("BBE-151 notice entry production boundary", () => {
  ...
  it("keeps the redirected board screen and writes on the same request-scoped graph", () => {
    ...
    expect(boardPage).toContain("applyNoticePerspective(loadedItems.map(projectNoticeMetadata), noticePerspective, ctx.user.id)");
    expect(boardPage).toContain("projectedItems.filter((item) => visibleItemIds.has(item.id)).map((item) => item.id)");
