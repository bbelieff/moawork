...
import { loadNoticeFileBytes, parseNoticeFile } from "@/lib/notices/official-file";
...
export async function GET(req: Request, { params }: RouteCtx) {
  ...
  const { repo, client } = await createRequestBoards();
  ...
  let bytes: Buffer;
  try {
    bytes = await loadNoticeFileBytes(stored, client);
  } catch {
    return Response.json({ error: "파일을 찾을 수 없습니다" }, { status: 404 });
  }
  return new Response(new Uint8Array(bytes), { headers: { "Content-Type": stored.mimeType, "Content-Disposition": `attachment; filename="${encodeURIComponent(stored.name)}"`, "Cache-Control": "private, no-store" } });
}
