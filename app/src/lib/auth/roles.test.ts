 ...
 *
 * ★★ 그리고 «모양이 넷» 이다. 처음엔 객체 지도 하나만 봤는데,
 *   **정작 그때 고친 위반 다섯 중 넷이 삼항·if 사슬이었다.**
 *   돌연변이 검증도 하필 «잡히는 유일한 모양» 을 밟아서 「잡는다」고 착각했다.
 *
 *       지도      owner: "대표"
 *       삼항      role === "owner" ? "대표" : …
 *       if        if (role === "owner") return "대표";
 *       고르는 칸  <option value="member">멤버</option>   ← 나중에 늘었다. 아래 ③
 *
 *   ★ 이 목록이 «셋» 이라고 적혀 있던 적이 있다 — 코드에 ③ 을 더해 놓고
 *     이 줄을 안 고쳤기 때문이다. **이 파일이 잡으려는 병을 이 파일이 앓았다.**
 *     패턴을 늘리면 여기도 같이 늘려라.
 *
 ...
 *       한 역할만 다시 적은 파일          임계값이 «역할 열쇠 둘 이상» 이라서 (오탐을 막는 대가다)
 *                                       — «횟수» 가 아니라 «가짓수» 다. admin 이 세 번 걸려도
 *                                          1가지라 안 터지고, admin+owner 면 2가지라 터진다
