...
export class ChecklistService {
  ...
  async applyProduct(dealId: string, productId: string) { const preset=await this.getPresetForProduct(productId); if(!preset) await this.setPreset(productId,[]); const next={dealId,productId,items:applyPreset(preset?.items??null)}; await this.store.saveDealChecklist(this.orgId,next); return next; }
