 ...
 *
 * 셀 값은 `item_values.column_key` 로 **컬럼 key 에 매달려** 있다. 그래서 값이 사라지는
 * 경로는 실질적으로 하나뿐이다 — 컬럼을 지우는 것. `repo/supabase/boardsRepo.ts` 의
 * `deleteColumn` 은 그 key 의 `item_values` 를 함께 DELETE 한다.
 *
