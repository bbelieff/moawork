...
export type ReportingHop = {
  userId: string;
  /** 담당자에서부터의 계통 거리. 직속은 1이다. */
  distance: number;
  /** 담당자부터 현재 수신자까지의 사용자 경로. */
  path: readonly string[];
};

/** BBE-124 알림 라우팅이 소비할 거리와 경로를 보존한다. */
export function reportingHopsOf(
  userId: string,
  ctx: ReportingContext,
  maxSteps?: number,
): ReportingHop[] {
  return reportingChainOf(userId, ctx, maxSteps).map((targetUserId, index, chain) => ({
    userId: targetUserId,
    distance: index + 1,
    path: [userId, ...chain.slice(0, index + 1)],
  }));
}
