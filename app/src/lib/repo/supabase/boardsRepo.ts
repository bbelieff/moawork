...
export class SupabaseBoardsRepo implements BoardsRepo {
  ...
  async updateItem(ctx:Ctx,id:string,patch:ItemPatch):Promise<BoardItem|undefined>{const{assigned_to,...safePatch}=patch;const q=await this.client.from("items").update({...safePatch,...(assigned_to!==undefined&&canSeeAll(ctx)?{assigned_to}:{}),updated_at:new Date().toISOString()}).eq("org_id",ctx.org.id).eq("id",id).select("*").maybeSingle();if(q.error)throw new Error(q.error.message);return(q.data??undefined)as BoardItem|undefined;}
  async deleteItem(ctx:Ctx,id:string):Promise<boolean>{const q=await this.client.from("items").delete().eq("org_id",ctx.org.id).eq("id",id).select("id");if(q.error)throw new Error(q.error.message);return(q.data?.length??0)>0;}
