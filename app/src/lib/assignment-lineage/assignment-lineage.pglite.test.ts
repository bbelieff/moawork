...
insert into public.deals(id,org_id,assigned_to,title) values
 ('${id.dealA}','${id.orgA}','${id.ownerA}','A'),('${id.dealB}','${id.orgB}','${id.ownerB}','B');
...
describe("#599 assignment lineage migration", () => {
  it("does not backfill customer rows and reports the current assignee as a read-only baseline", async () => {
    ...
    const read = await as<{ value: Record<string, unknown> }>(db, id.ownerA,
      `select public.read_assignment_lineage('${id.orgA}','${id.dealA}') value`);
  ...
  it("atomically changes deal, item, owner projection, one event and active-recipient notifications", async () => {
    const { db } = await boot();
    await as(db, id.ownerA, `select public.set_assignment_follower('${id.orgA}','${id.dealA}','${id.watcherA}',true,'${request(1)}')`);
    const call = `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.memberA}','${id.ownerA}',0,'${request(2)}') value`;
  ...
  it("rejects request mismatch, stale current/version, cross-org and inactive targets with zero partial change", async () => {
    const { db } = await boot();
    const call = `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.memberA}','${id.ownerA}',0,'${request(10)}')`;
    ...
    await expect(as(db, id.adminA, `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.ownerA}','${id.ownerA}',0,'${request(11)}')`)).rejects.toThrow(/version conflict/iu);
    await expect(as(db, id.adminA, `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.ownerB}','${id.memberA}',1,'${request(12)}')`)).rejects.toThrow(/active organization member/iu);
    await expect(as(db, id.adminA, `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.inactiveA}','${id.memberA}',1,'${request(13)}')`)).rejects.toThrow(/active organization member/iu);
    expect((await scalar<number>(db, "select count(*)::int n from assignment_transition_events")).n).toBe(1);
    expect((await db.query(`select assigned_to from deals where id='${id.dealA}'`)).rows).toEqual([{ assigned_to: id.memberA }]);
  });

  it("serializes competing expected-version writes so exactly one commits", async () => {
    const { db } = await boot();
    const attempts = await Promise.allSettled([
      as(db, id.ownerA, `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.memberA}','${id.ownerA}',0,'${request(20)}')`),
      as(db, id.ownerA, `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.watcherA}','${id.ownerA}',0,'${request(21)}')`),
    ]);
  ...
  it("keeps handoff events separate and leaves both ledger digests unchanged when a follower is removed", async () => {
    const { db } = await boot();
    await as(db, id.ownerA, `select public.set_assignment_follower('${id.orgA}','${id.dealA}','${id.watcherA}',true,'${request(30)}')`);
    const scheduled = await as<{ value: { handoffId: string } }>(db, id.ownerA,
      `select public.schedule_assignment_handoff('${id.orgA}','${id.dealA}','${id.memberA}','${id.ownerA}',0,'${request(31)}') value`);
    await as(db, id.ownerA, `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.memberA}','${id.ownerA}',0,'${request(32)}')`);
    ...
    await as(db, id.ownerA, `select public.set_assignment_follower('${id.orgA}','${id.dealA}','${id.watcherA}',false,'${request(33)}')`);
    ...
    await as(db, id.ownerA, `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.ownerA}','${id.memberA}',1,'${request(34)}')`);
  ...
  it("rejects inactive actors and cross-org reads, enforces read RLS, ACL and append-only triggers", async () => {
    const { db } = await boot();
    await expect(as(db, id.inactiveA, `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.memberA}','${id.ownerA}',0,'${request(40)}')`)).rejects.toThrow(/writer required/iu);
    await expect(as(db, id.ownerB, `select public.read_assignment_lineage('${id.orgA}','${id.dealA}')`)).rejects.toThrow(/unavailable/iu);
    await as(db, id.ownerA, `select public.reassign_deal_with_lineage('${id.orgA}','${id.dealA}','${id.memberA}','${id.ownerA}',0,'${request(41)}')`);
    ...
    const acl = await db.query<{ public_exec: boolean; anon_exec: boolean; service_exec: boolean; auth_exec: boolean }>(`select
      has_function_privilege('public','public.reassign_deal_with_lineage(uuid,uuid,uuid,uuid,bigint,uuid)','execute') public_exec,
      has_function_privilege('anon','public.reassign_deal_with_lineage(uuid,uuid,uuid,uuid,bigint,uuid)','execute') anon_exec,
      has_function_privilege('service_role','public.reassign_deal_with_lineage(uuid,uuid,uuid,uuid,bigint,uuid)','execute') service_exec,
      has_function_privilege('authenticated','public.reassign_deal_with_lineage(uuid,uuid,uuid,uuid,bigint,uuid)','execute') auth_exec`);
