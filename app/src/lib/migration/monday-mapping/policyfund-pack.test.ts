...
import { POLICYFUND_STRUCTURE_PACK, allSectionPresets } from "./policyfund-pack";
import {
...
describe("팩 구성 — 먼데이 실측 2026-08-05", () => {
  it("보드 3개다", () => {
    expect(POLICYFUND_STRUCTURE_PACK.boards.map((b) => b.slug)).toEqual([
      "newcust",
      "contact",
      "work",
    ]);
  ...
  it("아이템 프리셋(= 탭 안의 그룹)은 32종이다 — 14 + 7 + 11", () => {
    const [newcust, contact, work] = POLICYFUND_STRUCTURE_PACK.boards;
  ...
  it("그룹 순서는 보드마다 0부터 빈틈없이 이어진다", () => {
    for (const board of POLICYFUND_STRUCTURE_PACK.boards) {
...
describe("컬럼 계약", () => {
  ...
  it("실측 컬럼 수 — 신규고객 24 · 컨텍관리 21 · 업무관리 24 (Name·유예분 제외)", () => {
    expect(POLICYFUND_STRUCTURE_PACK.boards.map((b) => b.columns.length)).toEqual([24, 21, 24]);
  });
  ...
  it("optionRef 는 팩이 실제로 들고 있는 세트만 가리킨다", () => {
    for (const board of POLICYFUND_STRUCTURE_PACK.boards) {
      for (const column of board.columns) {
        ...
        expect(
          POLICYFUND_STRUCTURE_PACK.optionSets[column.optionRef],
...
describe("시드 확정 3건 — 2026-08-05 MW-총괄 (PLAN-002 §5 WO-1)", () => {
  ...
  it("② 지역은 두 보드가 같은 공용 세트 1개를 참조한다", () => {
    const refs = POLICYFUND_STRUCTURE_PACK.boards.flatMap((board) =>
      board.columns.filter((c) => c.label === "지역").map((c) => c.optionRef),
    );
    expect(refs).toEqual(["region", "region"]);
    expect(POLICYFUND_STRUCTURE_PACK.optionSets.region).toHaveLength(222);
  });
  ...
  it("D73 — 담당자별 슬롯 수는 원본 담당자 수와 같다(신규업체 2 · 컨텍관리 2 · 업무관리 0)", () => {
    const counts = POLICYFUND_STRUCTURE_PACK.boards.map(
...
describe("유예 컬럼 — 구조만 기록하고 설치하지 않는다 (PLAN-003)", () => {
  ...
  it("업무관리 수식 4종이 원문과 함께 기록돼 있다", () => {
    const work = POLICYFUND_STRUCTURE_PACK.boards[2];
...
describe("마이그레이션 040 과 앱 팩의 내용이 같다 — BBE-130/132", () => {
  // 팩이 SQL 과 TS 양쪽에 있으므로 한쪽만 고치면 조용히 어긋난다. 여기서 막는다.
  it("식별 키·출처 표기 외의 모든 중첩 내용이 완전히 일치한다", () => {
    ...
    const canonical = JSON.parse(JSON.stringify(POLICYFUND_STRUCTURE_PACK));
    const legacyPayload = { ...embedded };
    const canonicalPayload = { ...canonical };
    const legacySource = legacyPayload.source;
    const canonicalSource = canonicalPayload.source;
...
describe("팩 키 호환 별칭 — BBE-132", () => {
  it("새 키를 정본으로 쓰고 옛 키도 같은 팩으로 해석한다", () => {
    expect(POLICYFUND_STRUCTURE_PACK.key).toBe("pack.policyfund.v1");
    expect(LEGACY_POLICYFUND_PACK_KEYS).toEqual(["pack.seoul.policyfund1"]);
    expect(canonicalPolicyfundPackKey("pack.policyfund.v1")).toBe("pack.policyfund.v1");
    expect(canonicalPolicyfundPackKey("pack.seoul.policyfund1")).toBe("pack.policyfund.v1");
    expect(resolvePolicyfundPack("pack.seoul.policyfund1")).toBe(POLICYFUND_STRUCTURE_PACK);
    expect(resolvePolicyfundPack("pack.other.v1")).toBeNull();
  });
});
