...
describe("activityMetrics — DAU/WAU/MAU·스티키니스", () => {
  it("기간별 조직 최대 활성자를 합산한다", () => {
    const rows = [
      daily(daysAgo(1, NOW), "o1", { activeUsers: 3 }),
      daily(daysAgo(1, NOW), "o2", { activeUsers: 2 }),
      daily(daysAgo(4, NOW), "o1", { activeUsers: 5 }),
      daily(daysAgo(21, NOW), "o1", { activeUsers: 9 }),
    ];
    const m = activityMetrics(rows);
    expect(m.asOf).toBe(daysAgo(1, NOW));
    expect(m.dau).toBe(5); // 기준일: o1 3 + o2 2
    expect(m.wau).toBe(7); // 7일: o1 max 5 + o2 2
    expect(m.mau).toBe(11); // 30일: o1 max 9 + o2 2
    expect(m.stickiness).toBeCloseTo(5 / 11);
  });

  it("기준일은 오늘이 아니라 롤업이 채운 최신 날짜다 — 야간 배치는 전일까지만 집계한다", () => {
    // 오늘 행이 없어도 DAU 가 0으로 죽지 않아야 한다.
    const m = activityMetrics([daily(daysAgo(1, NOW), "o1", { activeUsers: 4 })]);
    expect(m.asOf).toBe(daysAgo(1, NOW));
    expect(m.dau).toBe(4);
  });

  it("데이터가 없으면 전부 0이고 기준일은 null 이다 (NaN 없음)", () => {
  ...
  it("기준일로부터 30일보다 오래된 데이터는 MAU 에 들어가지 않는다", () => {
    const m = activityMetrics([
      daily(daysAgo(1, NOW), "o1", { activeUsers: 1 }),
      daily(daysAgo(45, NOW), "o2", { activeUsers: 99 }),
    ]);
    expect(m.mau).toBe(1);
  });
});
