 ...
 *
 * ⚠ 기준일은 `now` 가 아니라 **데이터에 있는 가장 최근 날짜**다.
 *   야간 배치는 전일까지만 집계하므로 오늘로 창을 잡으면 DAU 가 항상 0이 된다.
 *   화면은 `asOf` 를 함께 표시해 어느 날 기준인지 밝힌다.
 */
export function activityMetrics(rows: readonly MetricsDailyRow[]): ActivityMetrics {
  const asOf = rows.reduce<string | null>(
    (max, r) => (r.date && (max === null || r.date > max) ? r.date : max),
    null,
  );
  if (asOf === null) {
    return { asOf: null, dau: 0, wau: 0, mau: 0, stickiness: 0 };
  }

  const anchor = Date.parse(`${asOf}T00:00:00.000Z`);
  const back = (n: number): string =>
    new Date(anchor - n * MS_PER_DAY).toISOString().slice(0, 10);

  const peakPerOrg = (from: string): number => {
    const byOrg = new Map<string, number>();
    for (const r of rows) {
      if (!inDateRange(r.date, from, asOf)) continue;
  ...
  const dau = peakPerOrg(asOf);
  return {
    asOf,
    dau,
    wau: peakPerOrg(back(6)),
    mau: peakPerOrg(back(29)),
    stickiness: ratio(dau, peakPerOrg(back(29))),
  };
}
