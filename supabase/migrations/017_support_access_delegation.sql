-- =====================================================================
-- 017_support_access_delegation.sql — T08 지원(1:1 문의) + 접근위임
-- 성격 : 001~016 위에 **additive**. 기존 파일·기존 정책 무수정.
...
-- 선행 : 001 … 016 (특히 006 = is_platform_admin, 015 = org 헬퍼 세션조건 제거 복구).
--        이 파일은 그 헬퍼들을 **읽기만** 하고 재정의하지 않는다.
-- 번호 : main 실측 최신(016) 다음. 최초 008 로 냈으나 그 사이 008~016 이 머지돼 재배정했다.
--        T06(mod.notify)도 같은 구간을 노리고 있으므로, 머지 순서에 따라 재배정될 수 있다.
-- =====================================================================
