-- =====================================================================
-- 036_preset_depersonalize.sql -- 프리셋에서 직원 실명 비우기 (BBE-130)
--
...
-- 성격 : 031 이 만든 structure_packs 테이블 -- 팩 1행을 update 만 한다. 새 테이블 -- 새 행
--        없음. 031 은 수정하지 않는다(기존 마이그레이션 무수정 -- 새 번호로만 추가).
-- 변경 : 담당자별 그룹 4종(신규업체 2 -- 컨텍관리 2)의 groupName 에서 직원 실명을 뺐다.
--        이모지 접두사만 남기고 앱 레이어(installStructurePack)가 설치 시 그 워크스페이스의
--        실제 멤버 이름을 붙인다(SectionPreset.assigneeSlot). 슬롯 수는 원본 담당자 수(2명)
--        그대로라 "아이템 프리셋 32종"은 불변이다. 담당자/협업자 컬럼 자체는 이미 PR #94
--        (BBE-46 수정)에서 person 타입으로 전환됐고 이번 마이그레이션은 건드리지 않는다.
-- 앱측 : app/src/lib/structure-packs/* 가 같은 데이터를 갖는다. 두 정의가 어긋나면
--        seoul-pack.test.ts 가 실패한다.
-- 선행 : 001 -> 002 -> 003 -> ... -> 035_ledger -> 036(이 파일)
-- =====================================================================

update structure_packs
set pack_jsonb = $json$
{
...
$json$::jsonb
where key = 'pack.seoul.policyfund1';
