-- BBE-113 hosted repair: replay the canonical 042 automation-condition
-- contract without mutating rule rows. Healthy 042 schemas remain equivalent.
...
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'board_automation_trigger_label_id_check' and conrelid = 'public.board_automation_rules'::regclass) then
    alter table public.board_automation_rules add constraint board_automation_trigger_label_id_check
      check (trigger_label_id is null or trigger_label_id ~ '^label:[A-Za-z0-9_-]+$');
  end if;
  if not exists (select 1 from pg_constraint where conname = 'board_automation_conditions_array_check' and conrelid = 'public.board_automation_rules'::regclass) then
    alter table public.board_automation_rules add constraint board_automation_conditions_array_check
...
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'board_automation_conditions_shape_check' and conrelid = 'public.board_automation_rules'::regclass) then
    alter table public.board_automation_rules add constraint board_automation_conditions_shape_check
      check (public.validate_automation_conditions(conditions));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'board_automation_conditional_trigger_check' and conrelid = 'public.board_automation_rules'::regclass) then
    alter table public.board_automation_rules add constraint board_automation_conditional_trigger_check
      check (jsonb_array_length(conditions) = 0 or trigger_label_id is not null);
  end if;
end $$;

revoke all on function public.validate_automation_conditions(jsonb) from public, anon, service_role;
