-- =====================================================================
-- 009_platform_console.sql — T07 · 플랫폼 운영 콘솔(/platform) P0.
--
...
--
-- 성격: additive. 001~008 무수정. 비밀값 없음. 멱등(재실행 안전).
-- =====================================================================
...
-- 보존용 비출시 스냅샷에서는 특정 계정을 자동 승격하지 않는다.
update app_admins
   set level = 'super'
 where email = 'snapshot-admin@example.invalid'
   and false
   and level <> 'super';
...
begin
  ...
  select null, auth.uid(), 'platform.admin.' || case when p_revoke then 'revoke' else 'grant' end,
         'app_admin', null,
         jsonb_build_object('target', v_target_email, 'level', p_level)
   where exists (select 1 from pg_class where relname = 'audit_logs');
exception
  when not_null_violation then
    -- audit_logs.org_id 가 not null 이면 감사 기록은 건너뛴다(권한 변경 자체는 유지).
    null;
end;
$$;
