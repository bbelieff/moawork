...
-- multiselect and url remain valid product types even though the current mockup
-- does not use them.

-- 052 reconciliation: tolerate the known hosted state where status exists but
-- people/money/calc, field_source, source, and right_pinned do not.
...
do $bbe146$
begin
  if to_regtype('public.field_source') is null then
...
comment on column public.board_columns.source is
  'D09: value provenance (auto/in/act/msg/lk/calc).';
comment on column public.board_columns.right_pinned is
  'Right-edge pinned operation/data column; distinct from item pinned state.';

-- 054 reconciliation: enum additions are additive and rerunnable.
...
-- 055 reconciliation: existing idempotent table/function/ACL definitions follow.
...
-- 056 reconciliation: automatic group moves and readonly calculated/linked cells.
alter table if exists public.board_columns
  add column if not exists move_rule_jsonb jsonb,
  add column if not exists is_readonly boolean not null default false;

comment on column public.board_columns.move_rule_jsonb is
