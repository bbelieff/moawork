...
begin
  ...
  if old.source='newcust.monday.v1:initializing' and new.source not in ('newcust.monday.v1:initializing','newcust.monday.v1') then
    raise exception 'NEWCUST_BOARD_SOURCE_TRANSITION_INVALID' using errcode='P0001';
  end if;
  return new;
end $$;
create trigger boards_newcust_source_guard before update of source on boards for each row execute function public.newcust_board_source_guard();
...
create policy boards_insert on boards for insert with check (public.is_org_member(org_id) and (source is null or source not in ('newcust.monday.v1','newcust.monday.v1:initializing') or public.org_role(org_id) in ('owner','admin')));
...
create policy boards_delete on boards for delete using (public.is_org_member(org_id) and (source is null or source not in ('newcust.monday.v1','newcust.monday.v1:initializing') or public.org_role(org_id) in ('owner','admin')));
...
create policy bgroups_insert on board_groups for insert with check (public.is_org_member(org_id) and (public.org_role(org_id) in ('owner','admin') or not exists(select 1 from public.boards b where b.id=board_id and b.org_id=board_groups.org_id and b.source in ('newcust.monday.v1','newcust.monday.v1:initializing'))));
...
create policy bgroups_delete on board_groups for delete using (public.is_org_member(org_id) and (public.org_role(org_id) in ('owner','admin') or not exists(select 1 from public.boards b where b.id=board_id and b.org_id=board_groups.org_id and b.source in ('newcust.monday.v1','newcust.monday.v1:initializing'))));
...
create policy bcols_insert on board_columns for insert with check (public.is_org_member(org_id) and (public.org_role(org_id) in ('owner','admin') or not exists(select 1 from public.boards b where b.id=board_id and b.org_id=board_columns.org_id and b.source in ('newcust.monday.v1','newcust.monday.v1:initializing'))));
...
create policy bcols_delete on board_columns for delete using (public.is_org_member(org_id) and (public.org_role(org_id) in ('owner','admin') or not exists(select 1 from public.boards b where b.id=board_id and b.org_id=board_columns.org_id and b.source in ('newcust.monday.v1','newcust.monday.v1:initializing'))));
