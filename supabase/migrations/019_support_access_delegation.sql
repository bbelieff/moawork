-- =====================================================================
-- 019_support_access_delegation.sql — T08 지원(1:1 문의) + 접근위임
-- 성격 : 001~018 위에 **additive**. 기존 파일·기존 정책 무수정.
...
-- 선행 : 001 … 018. 이 파일은 아래 헬퍼들을 **읽기만** 하고 재정의하지 않는다.
--        · 015_fix_org_helper_session_deadlock — is_org_member/org_role/org_scope 를 006 정의로 복구
--        · 017_fix_is_platform_admin_role_axis — is_platform_admin() 을 is_platform 단독 판정으로 수정
--          (T08 의 has_active_grant() 가 이 함수에 의존한다. 017 이전에는 항상 false 라
--           위임 수임이 프로덕션에서 무동작이었다 — T03 이 해소함)
-- 번호 : main 실측 최신(018) 다음. 최초 008 → 017 → 019 로 두 번 재배정했다.
--        머지 직전에 반드시 `git ls-tree origin/main -- supabase/migrations` 로 재실측할 것.
-- =====================================================================
