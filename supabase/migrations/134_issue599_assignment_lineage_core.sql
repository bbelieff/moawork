-- moa-migration-guard: logical_key=134_issue599_assignment_lineage_core predecessor=133_issue571_department_management_ui digest=d2bef12855d27df4358ff7c265b1f9ef43dc92b5a94baeba70ce7a92f459cd31 foundation=false

select public.begin_guarded_migration(
  p_logical_key => '134_issue599_assignment_lineage_core',
  p_file_name => '134_issue599_assignment_lineage_core.sql',
  p_file_digest => 'd2bef12855d27df4358ff7c265b1f9ef43dc92b5a94baeba70ce7a92f459cd31',
...
create table public.assignment_handoff_events (
  ...
  event_type text not null check (event_type in ('scheduled', 'cancelled', 'executed')),
  from_user_id uuid,
  to_user_id uuid not null,
  actor_user_id uuid not null,
  request_id uuid not null,
...
create or replace function public.assignment_lineage_require_writer(p_org_id uuid, p_deal_id uuid)
returns uuid
language plpgsql
security definer
set search_path = public, pg_temp
...
create or replace function public.read_assignment_lineage(p_org_id uuid, p_deal_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  select s.baseline_assignee_id, s.version into v_baseline, v_version
   ...
   where s.org_id = p_org_id and s.deal_id = p_deal_id;
  if not found then v_baseline := v_current; v_version := 0; end if;
  return jsonb_build_object(
    'orgId', p_org_id, 'dealId', p_deal_id, 'itemId', v_item,
    ...
    'transitions', coalesce((
      ...
      where e.org_id = p_org_id and e.deal_id = p_deal_id
    ), '[]'::jsonb),
    'followers', coalesce((
    ...
    'pendingHandoff', (
      ...
      where h.org_id = p_org_id and h.deal_id = p_deal_id and h.status = 'pending'
    )
  );
end;
$$;
...
begin
  v_actor := public.assignment_lineage_require_writer(p_org_id, p_deal_id);
  ...
  v_payload := jsonb_build_object('dealId', p_deal_id, 'assignedTo', p_assigned_to,
  ...
  select version into v_version from public.assignment_lineage_state
   where org_id = p_org_id and deal_id = p_deal_id for update;
  ...
  if v_current is distinct from p_assigned_to then
    ...
    update public.assignment_lineage_state set version = v_version, updated_at = clock_timestamp()
     where org_id = p_org_id and deal_id = p_deal_id;
    ...
    if found and v_pending.to_user_id = p_assigned_to then
      ...
      ) values (p_org_id, p_deal_id, v_item, v_pending.id, 'executed', v_pending.from_user_id, v_pending.to_user_id, v_actor, p_request_id);
    end if;
...
begin
  ...
  v_payload := jsonb_build_object('dealId', p_deal_id, 'userId', p_user_id, 'follow', p_follow);
  ...
  if v_replay is not null then return v_replay; end if;
  perform 1 from public.deals where org_id = p_org_id and id = p_deal_id for update;
  if not found then raise exception 'assignment projection unavailable' using errcode = '42501'; end if;
  if p_follow then
  ...
  select coalesce(version, 0) into v_version from public.assignment_lineage_state where org_id = p_org_id and deal_id = p_deal_id;
...
begin
  ...
  if p_expected_version is null or p_expected_version < 0 then raise exception 'expected version required' using errcode = '22023'; end if;
  v_payload := jsonb_build_object('dealId', p_deal_id, 'toUserId', p_to_user_id,
    'expectedAssignedTo', p_expected_assigned_to, 'expectedVersion', p_expected_version);
  ...
  select d.assigned_to, i.id into v_current, v_item from public.deals d
    join public.items i on i.org_id = d.org_id and i.deal_id = d.id and i.deleted_at is null
    where d.org_id = p_org_id and d.id = p_deal_id for update of d, i;
  if not found then raise exception 'assignment projection unavailable' using errcode = '42501'; end if;
...
begin
  v_actor := public.assignment_lineage_require_writer(p_org_id, p_deal_id);
  v_payload := jsonb_build_object('dealId', p_deal_id, 'handoffId', p_handoff_id);
  ...
  select * into v_row from public.assignment_pending_handoffs
   where id = p_handoff_id and org_id = p_org_id and deal_id = p_deal_id and status = 'pending' for update;
...
revoke all on function public.assignment_lineage_can_read(uuid, uuid),
  public.assignment_lineage_require_writer(uuid, uuid),
  public.assignment_lineage_require_active_member(uuid, uuid),
  public.assignment_lineage_replay(uuid, uuid, uuid, text, jsonb),
  public.read_assignment_lineage(uuid, uuid),
  public.reassign_deal_with_lineage(uuid, uuid, uuid, uuid, bigint, uuid),
  public.set_assignment_follower(uuid, uuid, uuid, boolean, uuid),
  public.schedule_assignment_handoff(uuid, uuid, uuid, uuid, bigint, uuid),
  public.cancel_assignment_handoff(uuid, uuid, uuid, uuid)
  from public, anon, authenticated, service_role;
grant execute on function public.read_assignment_lineage(uuid, uuid),
...
begin
  foreach v_function in array array[
    ...
    'public.read_assignment_lineage(uuid,uuid)'::regprocedure,
    'public.reassign_deal_with_lineage(uuid,uuid,uuid,uuid,bigint,uuid)'::regprocedure,
    'public.set_assignment_follower(uuid,uuid,uuid,boolean,uuid)'::regprocedure,
    'public.schedule_assignment_handoff(uuid,uuid,uuid,uuid,bigint,uuid)'::regprocedure,
    'public.cancel_assignment_handoff(uuid,uuid,uuid,uuid)'::regprocedure
  ] loop
    if exists (
