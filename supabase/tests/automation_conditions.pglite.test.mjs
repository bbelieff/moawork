...
const { PGlite } = await import(pathToFileURL(path.join(
  root, "node_modules", "@electric-sql", "pglite", "dist", "index.js",
)).href);

test("automation condition writes keep legacy rows and enforce tenant-safe stable IDs", async () => {
  const db = new PGlite();
  ...
  try {
    await db.exec(`
      ...
      grant select, insert, update, delete on public.board_automation_rules to authenticated;
      alter table public.board_automation_rules enable row level security;
      create policy bar_select on public.board_automation_rules for select
        to authenticated using (public.org_role(org_id) is not null);
      create policy bar_manage on public.board_automation_rules for all
      ...
      insert into public.board_groups values ('${group}','${board}');
      insert into public.board_automation_rules
        (id,org_id,board_id,status_column_key,status_value,to_group_id)
      values
        ('30000000-0000-4000-8000-000000000040','${org}','${board}','status','legacy-label','${group}');
    `);

    await db.exec(await readFile(path.join(
      root, "supabase", "migrations", "037_automation_cond.sql",
    ), "utf8"));

    const validatorAcl = await db.query(`select has_function_privilege(
      'authenticated', 'public.validate_automation_conditions(jsonb)', 'execute'
    ) as allowed`);
    assert.deepEqual(validatorAcl.rows, [{ allowed: true }]);
    ...
    await db.query(`insert into public.board_automation_rules
      ...
      values (
        '30000000-0000-4000-8000-000000000041','${org}','${board}','status','Done','label:done',
        '[{"column_key":"assignee","operator":"is","value_kind":"user_id","value":"user:owner"}]',
        '${group}'
      )`);
    ...
    await assert.rejects(db.query(`insert into public.board_automation_rules
      ...
      values ('30000000-0000-4000-8000-000000000043','${org}','${board}','status','Member','label:member','${group}')`),
    ...
    await assert.rejects(db.query(`insert into public.board_automation_rules
      (id,org_id,board_id,status_column_key,status_value,trigger_label_id,to_group_id)
      values ('30000000-0000-4000-8000-000000000044','${org}','${board}','status','Other','label:other','${group}')`),
    /row-level security/iu);

    await db.exec("reset role");
    const rows = await db.query("select status_value, trigger_label_id from public.board_automation_rules order by status_value");
    assert.deepEqual(rows.rows, [
      { status_value: "Done", trigger_label_id: "label:done" },
      { status_value: "legacy-label", trigger_label_id: null },
    ]);
  } finally {
    await db.close();
  }
