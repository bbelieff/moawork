...
import { inspectMigrationDirectory } from "../../scripts/check-migration-guards.mjs";
...
async function setup({ hostedDigest = "04e0b7f28387941c213a2c2758c8f751fde329cc17ce6c90213900daec00a4bc", includeLogo = true } = {}) {
  ...
  await db.exec(`insert into public.migration_apply_guard values
    ...
    ('099_bbe235_work_board_projection','099_bbe235_work_board_projection.sql','a289e67cad159ccc6937dfb6505d248993451d1b62c2ae12c204408ac5dbd6e8','098_bbe215_today_kpi_definitions','test','test',clock_timestamp()),
    ('100_bbe236_company_csv_import','100_bbe236_company_csv_import.sql','386eb1b07c46900a781e614864f563094193d9afe313c3a1427a267adf0cfd75','099_bbe235_work_board_projection','test','test',clock_timestamp()),
    ('101_bbe238_notice_board_unlock','101_bbe238_notice_board_unlock.sql','c83e6f3407646109c7b5bf3e387860ab61d40515f698d9fe16fe993eda62fe5a','100_bbe236_company_csv_import','test','test',clock_timestamp()),
    ('102_bbe239_board_item_files_storage','102_bbe239_board_item_files_storage.sql','555518005c270ce5ea9141fb13e1ded35f641144ac41863448fda32ad3811c38','101_bbe238_notice_board_unlock','test','test',clock_timestamp()),
    ('103_bbe240_deal_ledger_vat_wiring','103_bbe240_deal_ledger_vat_wiring.sql','866de2fb1741325d0f95b53c1160b0b05cbf3f90ba2a03f1a13bb2e60fcd4f0b','102_bbe239_board_item_files_storage','test','test',clock_timestamp()),
    ('104_bbe240_deals_fee_terms','104_bbe240_deals_fee_terms.sql','8b864d7609a18ea78eb59f04f038112c57f5a42ef7cb0f27f637296b84920205','103_bbe240_deal_ledger_vat_wiring','test','test',clock_timestamp()),
    ('105_bbe240_reserve_ledger_workspace_slug','105_bbe240_reserve_ledger_workspace_slug.sql','3d36f8d3eea65fa91c8354b78de620eb6b3aa73f8e5194c1bc19672aa5a6ca95','104_bbe240_deals_fee_terms','test','test',clock_timestamp()),
    ('106_bbe242_board_column_check_execute_restore','106_bbe242_board_column_check_execute_restore.sql','8f0c1215c0d87045490c09312fd73caa114c9eca7e90119b77dbc72cb9d06791','105_bbe240_reserve_ledger_workspace_slug','test','test',clock_timestamp()),
...
async function setupRepo() {
  ...
  await db.exec("create role anon; create role authenticated; create role service_role;");
  await db.exec(foundation);
  const inventory = await inspectMigrationDirectory(fileURLToPath(new URL("../migrations", import.meta.url)));
  for (const row of inventory.guarded.filter(({ logicalKey }) => Number(logicalKey.slice(0, 3)) >= 99 && Number(logicalKey.slice(0, 3)) <= 108)) {
    await db.query("insert into public.migration_apply_guard values ($1,$2,$3,$4,'test','test',clock_timestamp())", [row.logicalKey, row.fileName, row.digest, row.predecessor]);
  }
  return db;
}
...
test("joins a fresh repository frontier into the same canonical 109 successor", async () => {
  ...
  try {
    ...
    assert.deepEqual(row, { logical_key: "109_bbe268_migration_frontier_bridge", expected_predecessor: "108_bbe199_org_logo" });
