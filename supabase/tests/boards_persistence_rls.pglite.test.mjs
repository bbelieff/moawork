...
test("boards survive database restart and RLS isolates organizations", async () => {
  ...
  try {
    await db.exec(`
      ...
      create table board_views(id uuid primary key,org_id uuid not null,board_id uuid not null,name text not null);
    ...
    assert.equal((await db.query("select value_jsonb from item_values")).rows[0].value_jsonb, "ready");
    assert.equal((await db.query("select count(*)::int as n from board_views")).rows[0].n, 1);
