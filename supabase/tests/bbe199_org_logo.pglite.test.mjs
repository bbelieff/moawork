...
async function bootstrap(db, { withStorage = true } = {}) {
  ...
  await db.exec(`
    ...
    values (
      '107_bbe242_board_column_check_execute_restore',
      '107_bbe242_board_column_check_execute_restore.sql',
      repeat('0', 64),
      '106_bbe240_reserve_ledger_workspace_slug',
      'test-shortcut',
      'test-shortcut'
    );
  `);
  await db.exec(await migration("108_bbe199_org_logo.sql"));
}

test("108 은 additive 다 — 기존 row 를 건드리지 않고 로고 칸만 비어 있다", async () => {
  const db = new PGlite();
  try {
    await bootstrap(db);
...
test("108 을 두 번 적용하면 가드가 막는다", async () => {
  const db = new PGlite();
  try {
    await bootstrap(db);
    await assert.rejects(
      db.exec(await migration("108_bbe199_org_logo.sql")),
