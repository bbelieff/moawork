...
export class PostgresEsignRuntime implements EsignRequestLoader,EsignRequestStatusSink,SigningLinkDelivery{
 ...
 async loadPending(id:string):Promise<PendingEsignRequest|null>{const r=(await this.db.query<{request_id:string;org_id:string;deal_id:string;template_id:string;signer_reference:string;provider_document_id:string|null;provider_signing_url:string|null}>("select * from public.start_esign_delivery($1)",[id])).rows[0];return r?{requestId:r.request_id,orgId:r.org_id,dealId:r.deal_id,templateId:r.template_id,signerReference:r.signer_reference,...(r.provider_document_id&&r.provider_signing_url?{providerDocumentId:r.provider_document_id,signingUrl:r.provider_signing_url}:{})}:null;}
 async markProviderAccepted(id:string,doc:string,url:string){await this.db.query("select public.mark_esign_provider_accepted($1,$2,$3)",[id,doc,url]);}
