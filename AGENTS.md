...
- 구현·검수·후속 연계 작업은 **같은 물리 기기 레인**에서 닫는다. 데탑에서 시작한 작업은 작성자와 다른 데탑 LLM이 검수하고, 그 결과를 소비하는 후속 작업도 데탑 LLM에 배정한다. 노트북에서 시작한 작업은 같은 방식으로 노트북 LLM 안에서 닫는다.
- `작성자 ≠ 검수자` 원칙은 유지한다. 같은 기기란 같은 세션을 뜻하지 않는다. 지정 검수자가 없거나 응답하지 않으면 총괄은 같은 기기의 다른 유휴 세션으로 즉시 재배차하며, 다른 기기의 검수자를 기다리지 않는다.
- Linear/GitHub 댓글은 세션 호출 수단이 아니다. 총괄은 검수 세션을 직접 호출하고, 판정 ID를 확인한 뒤 owner 세션을 직접 다시 호출해 수정 또는 merge·배포 체인을 잇는다.
...
⑥ 동일 기기 독립검수(작성자≠검수자; 데탑→데탑, 노트북→노트북)
