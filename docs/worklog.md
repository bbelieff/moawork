...
## [모아워크 DC-00] 2026-08-20 — 검수: a3a1647 재실측 — 지시 3건 전부 PASS, 래칫 48=48, 워크로그 위치 1건 지적

- 커밋 `a3a1647`(계약업체 컬럼 3건)을 **직접 다시 재서** 검수했다(에이전트 보고를 그대로 믿지 않았다).
  단언이 아니라 **런타임 실측**으로 확인했다 — 임시 vitest 파일에서 `CONTRACT_WORK_TAB.columns` 를
  실제로 덤프해 28행을 눈으로 셌고, 확인 후 그 파일은 지웠다.
- **실측 컬럼 순서(런타임)**: `1 담당자(owner)` · `2 구분(engagement_kind)` · `3 진행기관(institution)` ·
  `4 상품명칭(fund_name)` · `5 세부명칭(product, 59선택지)` · 6~13 사업자유형…시군구 ·
  `14 진행상황(progress_status, rightPinned, 14선택지)` · 15~28 그대로. **총 28 · key 중복 0 · 라벨 중복 0.**
  ① 담당자 맨 앞 ✔ ② 세부명칭이 상품명칭 **바로 뒤** ✔ ③ 나머지 상대 순서는 이전 커밋에서 두 컬럼만
  빼고 다시 꽂은 것과 **완전히 일치** — 14번 이후 한 칸도 안 밀렸다 ✔.
- **라벨 조회가 안 깨졌다 — 조용한 빈 배열이 아니다**: `mappedOptions` 는 선택지가 비면 `[]` 를 돌려주는
  게 아니라 **throw 한다**(`contract-work.ts:40-42`). 그래도 실값을 찍었다 —
  `PROGRESS_OPTIONS_COUNT=14`, 14개 라벨 전량(대기중…업체관리)이 먼데이 사전과 동일했고
  `moveTo` 4규칙도 그대로 그룹을 가리킨다. 먼데이 사전(`POLICYFUND_WORK_BOARD`)에는 «진행상항» 만
  있고 «진행상황» 은 **없다**(실측 `ANY_진행상황_IN_MONDAY=false`) — 조회 키를 그대로 둔 판단이 옳다.
- **남은 «진행상항» 전수 확인**: 이관 사전 · 시드 JSON/마이그레이션 5곳 · presets/fixture/README/types ·
  목업 문서뿐이고 **CONTRACT_WORK_TAB 의 라벨로 조회하는 코드는 하나도 없다.**
  `/policyfund` 의 `stageColumn="진행상항"` 은 `getWorkBoardColumns(presets)`(시드 스냅샷)를 보지
  기본 탭을 보지 않는다 — 무관함을 코드로 확인했다. 비테스트 코드에 `columns[숫자]` 위치 의존도 **0건**.
- **게이트 재실행**: `rm -rf app/.next && bash scripts/check.sh` = **✅ check 통과**.
  영향권 스위트 단독 재실행(default-tabs · work-management · policyfund) 25파일 / **198건 전량 통과**.
- **래칫 재확인**: `node docs/design/qa-app.mjs` 단독 = `차이 합계: 48개 (천장 48) · 판정: DIFF`.
  qa-app 은 양방향 래칫이라 48=48 **정확히 일치** — 안 내린 천장이 숨어 있지 않다. `--self-test` 7/7.
  39→48 산식도 출력으로 검산했다: 앱에만 3→4(+1) · 목업에만 2→3(+1) · 자동이동 0→8(+8) ·
  선택지 6→5(−1) = **+9**. 커밋 메시지 본문과 `qa-app.mjs:31-39` 주석 양쪽에 근거가 적혀 있다.
- **커밋 범위**: 5파일뿐(contract-work.ts/test/render.test · qa-app.mjs · worklog). 딸려 들어온 무관 파일 없음.
  테스트는 약해지지 않고 **늘었다** — 0번 key 단언 하나가 `owner`/`engagement_kind`/`product`(4번) 셋이 됐다.
- **지적 1건(막지 않음)**: a3a1647 의 worklog 기록이 이 파일 **맨 아래**(`:2929`)에 붙었다.
  이 파일 규칙은 「최신을 위에」다 — 직전 검수(DC-00 2026-08-20)가 같은 지적을 했는데 반복됐다.
  지우지 않고 이 항목으로 위에 묶는다. 다음 커밋부터는 상단 삽입이다.
- `review-274-fix` 에 있고 **push 하지 않았다.** 실 화면을 연 증거는 여전히 없다(이 worktree 에 Supabase env 없음).
  기존 사용 중인 워크스페이스에는 이번 순서·라벨 변경이 **반영되지 않는다** — `ensureDefaultTabAdditive`
  에 `updateColumn`/`sort_order` 재배치가 없다는 직전 검수의 미결이 그대로 살아 있다.
...
## 2026-08-20 — 계약업체 실무 3건: 담당자 맨 앞 · 세부명칭을 상품명칭 뒤로 · 「진행상항」→「진행상황」 + 래칫 39→48

- 총괄 직접 지시 3건. **컬럼은 여전히 28개이고 key 는 한 글자도 바뀌지 않았다** — 순서와 라벨만 바뀐다.
  `item_values` 는 `column_key` 문자열로만 붙어 있고 FK 가 없어서(003_boards_engine.sql:64-70)
  key 를 바꾸는 순간 기존 셀 값이 전부 고아가 된다. 지난 커밋이 세운 규약을 그대로 지킨다.
- 순서: `owner` 를 4번에서 **1번**으로, `product` 를 13번에서 **5번**(상품명칭 바로 뒤)으로 옮겼다.
  둘을 빼서 앞에 다시 꽂은 것이라 **14번 이후는 한 칸도 밀리지 않는다** — 진행상황은 14번 자리와
  우측 고정을 그대로 유지한다.
- 라벨 «진행상항» → «진행상황» 은 **표시 라벨만** 고쳤다. 같은 컬럼의 `mappedOptions("진행상항")` 은
  그대로 둔다 — 그 인자는 라벨이 아니라 **먼데이 이관 매핑 사전의 조회 키**이고(policyfund-work.ts:112,
  먼데이 원본 컬럼 id `color`), 사전은 바깥 시스템이 실제로 뭐라고 적어 뒀는지의 기록이다.
  고치면 사전이 거짓말을 하게 되고 `mappedColumn` 이 **모듈 로드 시점에** throw 해서
  `@/lib/default-tabs` 를 import 하는 전부가 죽는다. 그 줄에 ⚠ 주석을 붙였다.
  `work-management/template.ts:14` 가 이미 같은 답을 갖고 있다 — 라벨 «진행상황» + `sourceAliases:["진행상항"]`.
  **제품 라벨은 고치고 출처 철자는 보존한다** 가 이 저장소의 규약이다.
- 따라서 «진행상항» 이 남아야 하는 자리는 전부 남겼다: 이관 사전 · `/policyfund` 재현 미리보기
  (`page.tsx:46` — 여기는 시드 스냅샷을 조회하고 못 찾아도 **조용히 0건**이 된다) ·
  `policyfund/presets.ts:32,60` · `fixture.ts:45` · 시드 JSON/마이그레이션 · 목업 v6(자기 내부 조회 키로 ~10곳).
- 래칫 **39 → 48**. 근거는 qa-app.mjs 주석에 적었다. 늘어난 9건은 전부 라벨 한 글자 차이의 파생이다 —
  목업에만 1 + 앱에만 1 + 자동이동 8(같은 4규칙이 라벨 절반만 달라져 목업만/앱만으로 갈라졌다)
  − 선택지 1(라벨이 안 맞아 대조가 건너뛴 부작용, 개선이 아니다).
  **컬럼 순서 재배치는 0건**이다 — qa-app 은 컬럼을 라벨 Map 으로만 대조하고 위치를 보지 않는다(:196-220).
  그룹은 순서까지 보지만 이번 변경은 그룹을 건드리지 않는다. `--self-test` 는 픽스처 기반이라 7/7 그대로다.
- 테스트: `contract-work.test.ts` 의 28라벨 순서 배열·우측 고정·moveTo 조회를 새 값으로 고쳤고,
  0번 key 단언을 `owner`/`engagement_kind`/`product`(4번) 셋으로 늘려 순서를 못박았다.
  `contract-work.render.test.tsx` 툴바 칩 목록도 같이. 단언을 **약하게 만들지 않았다**.

## 2026-08-20 — 운영 스키마 표류 해소: 마이그레이션 19건 적용 + `scripts/db-run.mjs` 신설

**무엇이 문제였나 — 원장이 진실이 아니었다.**
가드 원장(`migration_apply_guard`)에는 094·095·098_bbe199_org_logo 세 줄뿐이었다.
그런데 실제 운영 DB 를 재보니 096·097 의 «효과»(정책 6개·권한 3개·가드 함수)는 이미 서 있었다.
과거 운영이 막혔을 때 가드를 안 거치고 SQL 만 직접 실행한 흔적이다.
반대로 가드가 생기기 전(≤093) 마이그레이션 8건은 **아예 적용된 적이 없었다.**
즉 원장은 «적용된 것의 부분집합»도 «전체집합»도 아니었다 — 어느 방향으로도 믿을 수 없는 상태였다.

**어떻게 쟀나.** 저장소의 모든 `create table public.*` 을 뽑아 66개 기대 목록을 만들고,
`information_schema.tables` 와 대조해 «운영에 없는 것»만 남기는 쿼리를 돌렸다. 처음 14건이 나왔다.

**098 은 충돌이 아니었다.** 원장의 기본키는 파일번호가 아니라 `logical_key` 다(094:6).
`098_bbe199_org_logo` 와 `098_bbe215_today_kpi_definitions` 는 다른 키이고 `file_name` 도 다르다.
번호만 겹칠 뿐 DB 는 둘 다 받는다 — 「충돌이라 못 넣는다」고 본 것이 오진이었다.

**★ 스스로 만든 회귀 — 098 을 넣으면서 홈 화면을 깨뜨렸다.**
098 이 설치하는 `read_today_dashboard` 는 `onboarding_quest_progress`·`onboarding_quest_defs` 를
읽는다(098:267-268). 둘 다 048 이 만드는 테이블인데 운영에 없었다.
plpgsql 이라 **함수 생성은 통과하고 호출 시점에 42P01** 이 터진다. 그 함수를 앱 홈이 매 렌더마다 부른다.
098 을 적용하라고 하면서 런타임 의존성을 확인하지 않은 것이 원인이다.
→ 048 적용으로 복구. **교훈: 함수를 심는 마이그레이션은 그 함수가 참조하는 테이블의 존재를 같이 확인한다.**

**적용한 것(19건).**
- 가드 이전(원장에 안 남음): 0001 · 035 · 039 · 041 · 048 · 051 · 053 · 057 · 062
- 가드 이후(원장 기록됨): 096 · 097 · 098_bbe215 · 099 · 100 · 101 · 102 · 103 · 104 · 105
- 순서 제약 두 개를 실측으로 발견했다: 103 은 035(`deal_ledger_entries`)를, 057 은 051
  (`execute_board_automation_move`)을 각각 요구한다. 파일 번호만 보고는 알 수 없었다.

**데이터 변경은 0행이었다.** 053 은 `field_values`·`item_values` 의 전화번호를 실제로 고쳐 쓴다.
넣기 전에 운영 데이터량을 쟀다 — 워크스페이스 4개, 업체·아이템·셀값·필드값·딜·원장 전부 0건.
적용 결과도 모든 UPDATE 가 0행. 빈 DB 라 백필이 건드릴 것이 없었다.

**앱이 실제로 쓰는가를 먼저 쟀다.** 누락 13개 테이블에 대해 `.from(` / `.rpc(` 를 전수 조사한 결과
런타임에 도달하는 것은 3개뿐이었다(온보딩 2 + 자동화 요청 1). 나머지는 DB 트리거 전용이거나
호출자가 없는 죽은 경로였다(예: `messaging_opt_outs` 를 읽는 `createRetargetingCampaignService()` 는
부르는 데가 없다). 그래서 **048 을 1순위, 057 을 2순위**로 잡고 나머지는 뒤로 미뤘다.
최종적으로는 표류 자체를 없애려고 전부 넣었다 — 이번 사고의 원인이 곧 표류였기 때문이다.

**남긴 도구 — `scripts/db-run.mjs`.**
SQL Editor 복붙을 20번 반복하게 만든 것이 이번 작업의 진짜 비용이었다.
접속 문자열은 `.env` 의 `SUPABASE_DB_URL` 에서만 읽는다 — 인자로 받지 않는다(셸 히스토리 보호).
오류 메시지까지 포함해 접속 문자열을 출력에서 지운다(`redact`, :52-57).
`--whoami` / `--ledger` / `--file <path>` / `--sql "<query>"`.
세션 풀러(포트 5432)로 붙어도 `current_user` 가 `postgres` 로 잡히는 것을 확인했다 —
`begin_guarded_migration` 이 요구하는 조건이다.

**최종 상태.** 저장소 기대 테이블 66개 대비 **운영 누락 0건.** 가드 원장 13줄.
---

## [문서 인수 · 모아워크 DC 16] 2026-08-18 — #120 · #119 의 문서를 이어받는다

원 브랜치 두 개의 세션이 끊겨 리베이스에 필요한 force-push 를 할 수 없었다.
§9.1 에 따라 남의 브랜치를 만지지 않고, **문서만 새 브랜치로 옮긴다**(#247 에서 정한 방식).

- **#120** — 지역 시도/시군구 선행조사 설계문서 3종. ★ 조사 «뒤» 에 스키마가 움직여서
  각 문서 머리에 **「2026-08-12 기준」** 과 실측을 붙였다: `region_sido`·`region_sigungu`
  컬럼은 `065`(2026-08-15)에 **이미 있고**, 레이어 B 레거시 분해 규칙을 구현한
  마이그레이션은 **못 찾았다**. 조사가 틀린 게 아니라 그 사이 구현이 들어갔다는 뜻이다.
- **#119** — `docs/evidence/BRANCH-SYNC-2026-08-06.md`. 2026-08-06 시점 **기록물**이라
  「지금 저장소를 설명하지 않는다」를 머리에 붙였다.

아래 두 항목은 원 PR 의 worklog 기록을 **그대로** 옮긴 것이다(당시 시점의 기록).

### ↓ #120 원 기록 (2026-08-12)

## [인수 · 모아워크 DC 04/claude] 2026-08-12 — BBE-127 지역 분할 선행조사 인수·재구축

- 카드: BBE-127(전체 구현은 BBE-118 필터 대기 중 blocked — 아래 참고) · PR #120(`claude/bbe-127-region-survey`,
  2026-08-10 정지) 인수. belie 지시(각자 이름 근본으로 원래 배정받은 카드로 복귀) — 배정판상
  DC-04 소유 영역(리드컨택)의 두 번째 후보.
- 브랜치를 `feat/bbe-127-region-survey` 로 다시 세워 `origin/main`(`6adff30`) 위 rebase.
  43커밋 뒤처져 있었다 — 충돌은 `docs/worklog.md` 1건(append-only 위치 충돌)뿐, 문서 충돌 0.
  `컬럼표준·마이그레이션매핑_v1.md` 편집분을 diff 로 재확인 — **전부 추가(`+`)뿐**, 기존
  §0~§2·§3-1~3-4 원문 삭제·변경 0줄(원 작성자 주장과 일치).
- **손댄 것 없음** — 내용은 원안 그대로다. 코드 변경이 없어(문서 4개뿐) 이 카드는 원래도
  게이트 영향이 없다. `bash scripts/check.sh` **PASS** — app 1569/9skip · worker 85
  (rebase 로 최신 main 테스트 수 반영, 이 PR 자체의 신규 테스트는 0건 — 문서 카드).
- **왜 지금은 이 정도까지만인가** — BBE-127 «본 구현»(필터 결과 → 발송)은 선행 카드
  BBE-118(필터, DC-03 소유·In Progress)이 끝나야 착수 가능하다(카드 원문 "바닥층 4장이
  끝난 뒤 착수. 지금은 설계만"). 이 PR 은 그 설계 밑에 깔리는 **지역 분할 선행조사**만
  다룬다 — 완료된 조사를 그대로 최신 main 위로 옮기는 것이 이번 작업의 전부다.
- 검수 라우팅 — 작성자 DC 이므로 §5 기준 평시 짝 **DG-01**. 문서만 추가/편집이고 마이그레이션·
  발송·D24·구조정의 어느 것도 건드리지 않아 2단 승격 불필요로 판단.
- 판정 — **완주(문서 카드 범위 한정).** BBE-127 본 카드는 여전히 **Blocked by BBE-118** 이고,
  그 실제 구현(필터 대상 선택 → 발송 확인 화면 → 히스토리)은 이 PR 범위가 아니다. BBE-118 이
  끝나면 별도 카드로 착수한다.

## [END · 모아워크 노트북 CT01(260810)/claude] 2026-08-11(재개) — BBE-127 지역 시도/시군구 분할 재작업

- **결과**: 완주. 착수전 6단계 전부 실행(dump-mockup·qa-mockup 86/86·D65 원문 확보·SHA 실측 대조).
- **핵심 재발견**: "시군구가 없다"는 파싱 실패가 아니라 **D65 확정 구조**(신규리드=시도만 자동수집,
  시군구는 상담 중 사람이 직접 입력)다. 조사를 레이어 A(신규리드, 파싱 불필요)·레이어 B(계약된
  과거 데이터, 파싱 필요)로 재구성해 지난 라운드보다 정확해졌다.
- **산출물**:
  - `docs/design/지역사전_시도시군구_v1.md` — 시도 17 정답표. 목업(`dump-mockup.mjs new`)의 시도 17개
    순서·시군구 예시 12개와 전수 교차검증 완료.
  - `docs/design/BBE-127-지역분포조사_v1.md` — D65 직접 인용 + 레이어 A/B 구분 + 레거시 옵션(R1 217+1·
    R2 235) 형태 분류(재사용, 이전 라운드와 동일 실측) + 확인 필요 4건.
  - `docs/design/컬럼표준·마이그레이션매핑_v1.md` — **실재 문서 편집**(이전 라운드는 이 파일이 없는
    줄 알고 동명 신규 파일을 만들었는데, 이번엔 실물이 있어 그 방식 폐기). §3-4 뒤에 §3-5(지역 분해,
    레이어 A/B·우선순위 6규칙·절대금지) 추가, §4 표에 지역 관련 세는 항목 2행 추가, §7에 BBE-127
    반영 기록 추가. 기존 §0~§2·§3-1~3-4·§4 원문은 한 글자도 안 건드림.
- **수용기준 대조**: 형태별 건수 표 ✅ · 시도17+시군구 정답표 ✅(목업과 교차검증까지) · 우선순위
  규칙+실패처리 ✅ · "손으로 채워야 할 건" — 레이어 A는 **정의상 미상담 리드 전체**(정확한 건수는
  실 DB 필요, 쿼리 남김)·레이어 B는 **13건 이상**으로 숫자 제시 ✅ · BBE-123·125·118·**109** 통지 예정.
- **리스 반납**: 문서 4개(신규 2·수정 2: 컬럼표준+worklog) 외 무변경. `app/**`·`worker/**`·`supabase/**` 0건.
- **PR**: 코드 0 → 배포·health NOT_RUN(해당없음). 데탑 CT09 검수 요청.

## [START · 모아워크 노트북 CT01(260810)/claude] 2026-08-11(재개) — BBE-127 지역 시도/시군구 분할 재작업

- **재개 사유**: 이전 라운드(PR #120, 브랜치 `claude/bbe-127-region-survey`)에서 착수전 5단계 문서 5개 중
  4개가 저장소에 없다고 보고했는데, 이번 배정 시점엔 전부 존재함(`dump-mockup.mjs`·`qa-mockup.mjs`·
  `docs/plans/배정-260810/21_노트북CT01.md`·`docs/handoff/결정대장.md`·`컬럼표준·마이그레이션매핑_v1.md`).
  실측을 다시 했다.
- **착수전 6단계 실행 결과**:
  - ① `node docs/design/dump-mockup.mjs new` 실행 완료 — 목업을 브라우저 대신 텍스트로 읽음.
    **목업 자체가 지시를 내장하고 있음**: "시군구 예시 12개 → 시도별 전체 사전으로"(19행) — 이번 조사가
    정확히 이 요구를 채우는 일임을 재확인.
  - ② `node docs/design/qa-mockup.mjs` → **86/86 통과**(55도 75도 아님, 지시대로 확인).
  - ③ `docs/handoff/결정대장.md` D65 원문 확보(추측 아님, 직접 인용):
    > **D65** — 지역은 2칸. 시도는 광고에서 수집(⟳), **시군구는 상담하며 입력(✎)**.
    > *"그냥 서울로만 나와요… 다시 상담했을 때 서울 관악구다 하면 관악구로 설정을 해 주셔야"* —
    > 특례보증은 지자체 단위라 시군구 없으면 D63(BBE-127 본 카드)을 못 한다.
    → **핵심 재발견**: "시군구가 없다"는 파싱 실패가 아니라 **구조적으로 원래 없다**(신규리드 단계는
    광고 폼이 시도까지만 준다. 시군구는 상담 전화에서 사람이 direct로 채운다). 이전 라운드의
    "파싱 규칙으로 쪼갠다"는 프레이밍은 신규리드 단에는 안 맞고, **이미 상담을 거쳐 계약된 과거
    데이터**(업무관리·회계 보드의 결합형 「지역」 드롭다운, R1/R2)에만 적용되는 이야기였다.
    두 레이어를 분리해 문서에 반영한다.
  - ④ `git fetch origin && git rev-parse origin/main` = `c32e01e`(지시받은 SHA와 일치, 실측 확인).
  - ⑤ 리스 선언(아래) 후 착수. BBE-94 착수 도장은 이번 메시지 하단 참고(재개라 별도 착수 도장은
    생략 — 이미 진행중 상태로 배정판에 반영돼 있음, 완주 시 도장 예정).
- **배정판(`00_배정판.md`) 대조**: BBE-109(노트북 GT01)가 "노트북 CT01의 지역 분해 규칙 조사"를
  대기 중임을 확인 — 이번 라운드 통지 대상에 BBE-109가 추가된 이유. 다른 슬롯과 파일 충돌 없음.
- **리스**: `docs/design/지역사전_시도시군구_v1.md`(신규) · `docs/design/BBE-127-지역분포조사_v1.md`(신규) ·
  `docs/design/컬럼표준·마이그레이션매핑_v1.md`(**기존 실재 문서** — §3-2·§4를 덮어쓰지 않고, 그 절 스타일에
  맞춰 지역 전용 하위 절과 §4 표 행을 추가하는 방식으로 편집. 이전 라운드는 이 문서가 없는 줄 알고
  동명의 새 파일을 만들었는데, 이번엔 진짜 문서가 있어 그 방식은 폐기).
- **안 만지는 것**: `app/**`·`worker/**`·`supabase/**`, 결정대장(제안만).
- **이전 PR #120 처리**: 브랜치를 origin/main 기준으로 리셋하고 새로 커밋한다(구 버전 컬럼표준 파일은
  실제 문서와 충돌해 폐기). PR #120은 강제 갱신되거나, 필요 시 닫고 새 PR로 대체한다.
...
## 2026-08-20 — BBE-242 「096 은 작동한 적이 없다」 + Vercel 리전 icn1 + main 머지

**증상.** 운영과 프리뷰 «양쪽» 에서 신규리드 탭이 `This page couldn't load` 로 죽었다.
양쪽이라는 것이 단서였다 — DB↔앱 세대 불일치가 아니라 «진짜 버그» 라는 뜻이다.

**재현.** 운영 DB 에 실사용자 권한으로 붙어 앱과 같은 쓰기를 흉내 냈다(롤백 전제).
```
insert into public.board_columns (...) → ERROR 42501
  permission denied for function board_column_metadata_is_valid
```

**원인 — 두 마이그레이션이 각자 옳고 «합쳐서» 틀렸다.**
- `089:53-55` 가 `board_columns` 에 CHECK 제약을 걸고 그 안에서 `board_column_metadata_is_valid()` 를 부른다.
- `092:15-19` 가 「최소권한」 취지로 그 함수의 EXECUTE 를 `authenticated` 에서 회수했다.

★ **CHECK 제약이 함수를 부르면 그 함수는 «쓰는 역할» 의 권한으로 실행된다.**
  EXECUTE 가 없으면 제약이 «검증» 이 아니라 **«전면 차단»** 이 된다.
  092 는 「검증기를 직접 못 부르게 한다」고 생각했지만 실제로는
  「그 테이블에 아무것도 못 쓰게 한다」를 했다.

**파급.** 로그인 사용자가 보드 컬럼을 한 개도 못 썼다. 그래서 데모 조직의
「신규리드 관리」 보드는 **컬럼 0 개인 껍데기**로 남았고(공지사항 10 · 데모 2 대비),
기본 탭 4 개 중 2 개는 아예 만들어지지 못했다. 그 보드를 그리는 화면이 서버 오류로 죽었다.

**★★ 왜 아무도 못 잡았나 — 이 카드의 본체.**
`096` 이 바로 이걸 고치려던 마이그레이션이었다. 테이블 GRANT 와 RLS 정책을 되살렸고,
주석에 「CHECK 제약이 남아 있으니 직접 써도 잘못된 JSON 은 못 들어간다 — 위험은 그만큼 좁다」
고 적었다. **그 문장은 맞다. 다만 그 제약이 «옳은 JSON 까지» 막고 있다는 것을 보지 못했다.**
그리고 096 의 검증 블록은 ① 테이블 권한 ② 정책 이름+cmd ③ RLS 활성 을 확인한다 —
**셋 다 통과했는데 쓰기는 안 됐다.** 전제 조건만 재고 «동작» 을 한 번도 안 해봤기 때문이다.
→ `106` 의 검증은 권한 비트뿐 아니라 **진짜 INSERT 를 해보고 지운다.**

**고침(106).** 검증기 3종의 EXECUTE 를 `authenticated` 에 돌려준다.
셋인 이유: `metadata_is_valid` 는 SECURITY INVOKER 라 본문에서 부르는 나머지 둘도
호출자 권한으로 돈다 — 하나라도 빠지면 같은 42501 이 난다.
여는 범위는 좁다: 셋 다 jsonb 모양만 보는 순수 검증 함수이고 테이블을 읽지도 쓰지도 않는다.
조직 경계는 그대로 RLS `bcols_*` 의 `is_org_member(org_id)` 가 지킨다.
제약에 `comment` 를 달아 «다시 회수하려면 제약부터 걷어내라» 를 그 자리에 남겼다.

**Vercel 리전 — 느림의 정체.**
`app/vercel.json` 에 `regions` 키가 없었다 → 기본값 `iad1`(워싱턴 D.C.).
DB 는 `ap-northeast-2`(서울)다. **DB 왕복마다 태평양을 건넜다.**
저장소에 이미 있던 실측이 규모를 말한다(`page.roundtrips.test.tsx`, BBE-214):
**탭 클릭 한 번 = 왕복 33 회 · 직렬 18 단계.** 직렬 18 × 대륙간 왕복 ≈ 수 초.
데이터가 0 건이어도 안 줄어드는 이유다. → `regions: ["icn1"]` (DB 와 같은 AWS 리전).
★ 직렬 18 단계 자체는 그대로다 — 데이터가 쌓이면 다시 아프다. 별건으로 잡는다.

**머지가 드러낸 것.** main 이 새로 들여온 래칫(`result-notice.test.ts`, BBE-208)이
우리 코드를 잡았다 — `LedgerEntryModal` 이 `setMessage(result.message)` 로 판정을 담아 놓고
표현은 `role="alert"` + 빨간색으로 못박아 뒀다. **KNOWN_REMAINING 에 이름을 올려 통과시키지 않았다.**
상태를 `ResultNotice({ok,message})` 로 바꾸고 공용 `ResultBanner` 를 쓴다 —
role·aria-live·색이 한 곳에서 함께 `ok` 를 읽는다.
