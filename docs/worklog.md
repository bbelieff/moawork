...
## 2026-08-25 — #531 업체관리 현황을 목업 13열로 다시 세우다 (수수료율 → 계약조건)

- base `origin/main@63ba4b2`, branch `claude/issue-531-company-status-layout`.
- **총괄 직접 지시**: 「수수료율을 계약조건으로 하자」. 목업 `T.company` 의 8번째 열
  `<th class="num">수수료율</th>`(`${d.rate}%`) 자리를 **계약조건(자유기재)** 으로 둔다.
  저장 위치는 이미 있었다 — `deals.fee_terms`(마이그레이션 105_bbe240, 운영 DB 적용 확인).
  → `CLAUDE.md` 「기준의 우선순위」 1번이 2번(목업)을 이기는 자리라서 **목업과의 영구 차이 1건**으로 등록한다.
- **화면이 목업과 아예 다른 물건이었다**: 기존 `CompaniesWorkspace` 는 5칸 카드 목록이고,
  목업은 KPI 5 · 필터 6 · 패널 제목 · **13열 표** 다. 전부 옮겼다.
- **자리 틀림을 한 번 스스로 잡았다**: 처음 구현에서 회사 줄의 «진행기관» 칸에 지역(서울)을,
  «담당자» 칸에 대표자를 넣었다. 목업은 그 둘을 열이 아니라 **첫 칸 아래 작은 줄**
  (`대표 · 업종 · 지역`)에 붙이고, 2·3번 칸에는 **건들의 진행기관·담당자 고유값**을 넣는다.
  배치 계약에 이 자리를 못박아 다시 돌아오지 못하게 했다.
- **배치 계약 11건**(`CompaniesWorkspace.layout.test.tsx`) — 총괄이 EVAL 핵심으로 지목한
  「있어야 할 자리에 그것이 있는가」를 잰다: ①존재(13열·KPI 5·필터 6) ②좌→우 순서
  ③세로 순서(제목→KPI→필터→패널 제목→표) ④시각 속성(`sticky left-0`·`overflow-x-auto`).
  못 재는 것도 적어 뒀다 — 여백·색·글꼴·반응형 붕괴·요소 겹침은 완주 전 «눈으로 한 번» 의 몫이다.
  ★ 열 순서는 `<thead>` 안에서만 잰다. 「담당자」가 필터에도 있어서 문서 전체로 재면 어긋난다.
- **접힘을 `hidden` 으로 바꿨다**. 이 화면은 원래 `<details>` 였고, 접혀도 자금 건 내용이 문서에
  남아 브라우저 찾기·보조기술이 찾을 수 있었다. 표에서는 `<details>` 로 `<tr>` 을 감쌀 수 없어
  상태로 바꿨는데, 조건부 렌더로 지우면 그 성질을 조용히 잃는다. 기존 e2e 가 이걸 잡아 줬다.
- 원장은 합계만 넘기던 것을 **entry 원본까지** 넘기게 했다. 표가 그리는 것은 합계가 아니라
  건별 계약금·수수료·수납일이다. 미수금은 여전히 entry 단위로 더한다(순합계로 빼면 VAT 초과입금
  한 건이 다른 건의 진짜 미수금을 0원으로 가린다).
- 담당자·상태 이름표는 **못 읽어도 화면이 선다**(그 칸만 빈다). 금액은 정반대로, 실패를 0원으로
  바꾸지 않고 «확인 필요» 를 그대로 들고 간다.
- **채우지 못한 칸 2개를 숨기지 않았다**: ②진행기관 — `deals` 에 없고 계약업체 실무 보드의
  `institution` 에만 있다. 복사하면 같은 값이 두 곳에 저장돼 표류하므로 비웠다.
  ⑥승인 — 승인**일** 필드가 제품 어디에도 없다. 헤더는 목업 자리 그대로 두고 값만 «—» 다.
- **같이 발견한 어긋남(이번 PR 에서는 안 건드림)**: `contract-work.ts:119` 에 아직
  `fee_percent`「수수료(%)」와 계산열 `fee_amount`「ƒ수수료(원)」가 살아 있다. 총괄 지시는 이쪽도
  계약조건으로 가자는 쪽이지만, %를 없애면 계산열이 성립하지 않고 그 정본은 원장이어야 한다.
  입력 흐름을 바꾸는 일이라 별건으로 확인받는다. → #531 코멘트에 후속 항목으로 남김.
- 검증: 회사 관련 33 tests PASS. hosted 적용·고객 데이터 쓰기·비밀값 조회는 0건이다.
...
## [코디네이터] 2026-08-20 — 원칙·매뉴얼 일원화 (문서 전용)

**왜** — 「정본」을 자칭하는 문서가 다섯 곳이었고 서로 반대로 말했다. 워커가 무엇을 따라야 하는지
판단할 근거가 없었다. 특히 워커에게 실제로 붙여넣어지는 `docs/playbooks/worker-onboarding.md` 가
`AGENTS.md §10` 이 폐기한 것들을 그대로 지시하고 있었다.

**실측한 모순**

- 「정본」 자칭 4곳 — `CLAUDE.md:42` · `AGENTS.md:1` · `AGENTS.md:145` · `배정판.md:5`.
  「착수 금지」가 두 곳에 걸려 규칙이 서로를 막고 있었다. (+ `worker-onboarding.md:1` 이 다섯 번째)
- 반장(01) — `AGENTS.md:101` 「폐지했다」 vs `배정판.md:13` 「총괄 1 + 반장 1 + 작업원 4」.
  **둘 다 2026-08-12 같은 커밋**에 들어갔다. 같은 파일 안에도 `AGENTS.md:128` 「01 ← 반장 겸 검수자」가 남아 있었다.
- 편제표는 24칸인데 Orca 실측 결과 살아 있는 에이전트는 1개. 나머지 8개 터미널은 빈 셸.
- 「문서를 만들지 마라」(`AGENTS.md:149`)고 적어 놓고 `docs/**` 에 166개(archive 별도 40개).
- `worker-onboarding.md` 가 지시하던 것 중 이미 폐기된 것 — 「촬영 불가는 머지 중단 사유가 아니다」
  (2026-08-11 P0 의 직접 원인) · 「375px 검증 면제」 · `CT01~10` 세션명 · 「마이그레이션 최신 030」(실제 097) ·
  저장소에 없는 `docs/plans/naming-rules.md`·`onboarding-moawork.md` 참조.

**한 것**

- `CLAUDE.md` — 「무엇을 만드는가」로 축소. 정본 표 추가. **네 진영(데탑·노트북 × 클로드·코덱스) 적용 범위**
  명시. 맨 끝에 `@AGENTS.md` 임포트를 달아 클로드가 규칙을 자동으로 끌어오게 했다.
- `AGENTS.md` — 「일하는 방식」 유일 정본. §0(먼저 읽을 것) 신설 · §1 을 〔기록〕으로 강등 ·
  §2 편제를 «코디네이터 1 + 워커 N» 으로 전면 교체(반·칸·진영 축 폐지) · §4 배정 정본을 Linear 로 ·
  §5~§9 의 반장·짝반장·2단검수·기기레인·MWC 잔여 제거 · §10 폐기 목록에 이번 분과 **미결 5건** 추가.
  코덱스는 `CLAUDE.md` 가 자동으로 안 읽히므로 맨 위에서 직접 열라고 지시.
- `docs/playbooks/worker-onboarding.md` — 전면 교체. 규칙을 갖지 않고 두 정본을 가리키는 킥오프 양식 +
  실사고 함정 14건(F11 hosted 장부 · F12 worklog 충돌 · F13 fetch refspec · F14 브랜치 보호 없음 추가).
- `docs/coordination/**` · `docs/plans/README.md` 11개 파일에 〔기록〕 배너. **지우지 않았다** —
  지우면 같은 시행착오를 다시 하기 때문이다.

**모순 하나를 새로 만들었다가 잡았다** — §2.1 에 「워커는 다음 일을 스스로 시작하지 않는다」를 쓰고도
§8.2 에 구판의 「끝나면 다음 카드를 스스로 집는다」가 남아 있었다. §8 을 같이 고쳐 «보고하고 턴을 끝낸다» 로 통일했다.

**남긴 것 (belie 결정 대기 · `AGENTS.md §10.3`)** — 라벨 체계 교체 · `worklog.md` 분할(266 KB, PR 충돌의
유일한 원인) · SYNC ROUND-1~34 archive 이동 · 죽은 터미널/워크트리 정리 ·
