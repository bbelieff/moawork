...
## [인수+FIX · 모아워크 DC 04/claude] 2026-08-12 — BBE-117 뷰 시스템 인수 · 참조 컬럼 소실 방어

> ⚠️ 작성 당시 서명이 잠시 «NC 04» 였던 것을 belie 재확인으로 **DC 04** 로 되돌렸다
> (NC 반은 별도로 만들 예정). PR #121 이 아직 미병합이고 이 항목의 유일한 작성자가 나라
> 새 정정 항목을 덧붙이지 않고 직접 고쳤다 — 다른 세션이 이 위에 뭔가를 쌓은 적이 없다.
...
- 검수 라우팅 — 작성자 DC 이므로 §5 기준 평시 짝 **DG-01** · 2단(구조/D24 해당 시) **NG-01**.
  이 카드는 저장 형식·도메인 로직만 건드려 2단 조항(돈·마이그레이션 파일추가·D24·구조정의)
  ①·③(마이그레이션 파일 추가 + D24 조회범위, 1단 검수가 실측)에 해당 — NG-01 필요.
