...
## 2026-08-12 · BBE-46 워크스페이스 생성 시 구조 자동 지급 (NC-03)

- **base** `origin/main@6adff30` · 브랜치 `claude/bbe-46-creation-seed`
- 카드 원문(2026-08-05)은 낡아서 **AGENTS.md 2026-08-12판 + D76 으로 재해석**해 수행했다.

### 실측 — 만들 것은 «팩» 이 아니라 «연결» 이었다

```
SEOUL_STRUCTURE_PACK   🔥신규고객 24컬럼/14그룹 · 🔥컨텍관리 21/7 · 🔥업무관리 24/11 = 32그룹
                       → 카드가 요구한 «아이템 프리셋 32종» 은 이미 main 에 있다(PR #94 계열)
installStructurePack() 호출부 2곳 — 둘 다 «생성» 경로가 아니다
  ① boards/InstallPackButton.tsx   D76 이 폐기하는 «설치» 버튼
  ② lib/onboarding/server.ts        연습 회사 전용
```

그래서 새 회사를 만들면 탭이 0개였고 화면은 «구조 팩을 설치하면» 안내만 띄웠다 —
belie 가 프로덕션에서 부딪힌 막다른 길(진단 §1 · 인수인계 §1).

### 넣은 것 — `app/src/lib/workspace-bootstrap/` (신규)

`bootstrapNewWorkspace(ctx)` 하나. 앱에서 워크스페이스를 만드는 경로는
`getRepo().createOrg()` **하나뿐**(실측)이라 그 직후에 한 줄로 붙였다.
**체크박스 뒤에 두지 않았다 — 고르는 것이 아니라 기본값이다(D76).**
멱등하다(같은 이름 보드는 건너뛴다) — 재시도돼도 두 벌 생기지 않는다.

앞으로 «새 회사가 무엇을 갖고 시작하는가» 는 이 파일 하나만 고치면 된다.
BBE-145 의 `default-tabs` 도 나중에 여기로 합류시키면 된다.

### 수용 기준 (D76 재해석) — 테스트 12건

```
보드 3개 팩 순서·이름 그대로            ✅
컬럼 69개(24+21+24) 순서 그대로          ✅  구조 축소 0 (D73)
그룹 색 hex · 순서 0부터 빈틈없이        ✅  «색·순서까지» 수용기준
★ 데이터 0 — 행을 만들지 않는다          ✅  D72
멱등 · 조직 경계 격리                    ✅
유예 컬럼(PLAN-003)은 만들지 않는다      ✅  없는 것을 있는 척하지 않는다
D71~D75 예시 사람·업체 이름 0            ✅  그룹명·선택지 전수 검사
★ 멤버 1명 → 담당자별 그룹도 1개          ✅  그게 «빈 상태» 다 (D73 · BBE-130)
```

> 테스트는 팩을 직접 부르지 않고 **실제 생성 경로(`createOrg` → bootstrap)를 그대로 태운다.**
> 그러지 않으면 «팩이 잘 도는가» 만 증명하고 «생성하면 있는가» 는 증명하지 못한다 —
> 진단서 §1.2 의 「목업 86/86 PASS」와 같은 종류의 착각이 된다.

### 일부러 하지 않은 것

- **«설치» 버튼을 지우지 않았다.** D76 이 개념을 폐기했지만, **이 변경은 새 워크스페이스만
  고친다.** 이미 만들어진 워크스페이스(=belie 의 프로덕션 회사)는 여전히 보드가 0개이고
  그 버튼이 유일한 복구 수단이다. 지금 지우면 그 회사들이 갇힌다.
  → **후속 필요: 기존 빈 워크스페이스 백필.** 그게 끝나야 버튼을 지울 수 있다.
- **`/newcust` 의 막다른 안내문을 고치지 않았다.** *"회사 관리자가 구조 팩 설치를 완료하면"*
  이라고만 하고 링크가 없다 — belie 가 실제로 본 화면이 이것이다. 설치 버튼은
  `/boards` 에만 있어서 거기까지 못 가면 길이 없다. 다만 그 라우트는 6탭 셸(BBE-142 · #166)
  소유라 손대지 않았다 — 직전 라운드에 같은 자리에서 env 가드를 중복으로 만든 적이 있다.
- **BBE-146 대기 아님** — 팩의 `PackColumn` 은 `source`·`move_rule_jsonb`·`is_readonly`·
  `right_pinned` 를 쓰지 않는다(실측). 그 필드는 `default-tabs`(BBE-145) 쪽이다.
- **마이그레이션 0개** — 팩은 TS 상수라 SQL 이 필요 없다. 번호 경합(059/060) 무관.

### 화면 확인 — ❌ NOT_RUN (BBE-145 와 **같은 원인**)

`/boards` 는 `loadPermGuard("work.view_tabs")` → Supabase RPC `effective_permission` 을 부르고
`lib/perm/server.ts` 가 조회 실패를 «불허» 로 수렴시킨다(의도된 fail-closed). env 없는 로컬에서
항상 `notFound()`. 게다가 이 세션은 Browser pane 미표시로 촬영 자체가 불가하다.

> ★ **이것이 두 카드 연속 같은 벽이다. 개별 카드의 문제가 아니라 구조 결함이다.**
> C 진영은 §3 이 요구하는 «눈으로 본 증거» 를 로컬에서 만들 수단이 없다.
> **역제안 — DG-04 에 후속 카드**: `LocalBoardsRepo` 와 같은 패턴으로
> `hasSupabaseEnv()` 가 false 일 때만 쓰이는 **로컬 전용 권한 어댑터**를 포트 뒤에 끼운다.
> 프로덕션 fail-closed 는 그대로 두고(약화 금지) 로컬에서만 owner=전체 허용으로 판정한다.
> 이게 없으면 앞으로 모든 화면 카드가 같은 자리에서 NOT_RUN 을 반복한다.
