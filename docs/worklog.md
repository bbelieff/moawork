...
>
> ★ **다시 정정(같은 날 저녁) — 최종 확정은 DC-03 이다.** belie: *"내가 너 원격인지
> 몰랐다. NC 는 따로 만들 것이니 너는 DC 03 으로 돌아가."* 위 두 문단은 그 사이의
> 일시적 정정 기록이라 지우지 않고 남긴다(append-only). 검수 경로도 §5 DC 기준
> (1단 DG-01 · 2단 NG-01 · 비상 NC-01)으로 되돌아갔다 — BBE-94 참고.

- **base** `origin/main@7733876` · 브랜치 `worktree-bbe-145-newlead`
- **착수 전 qa-app 차이 175개 / 완료 후 175개** (안 움직였다 — 이유는 아래 «측정» 항)

### 무엇을 만들었나 — `app/src/lib/default-tabs/` (신규)

목업 v6 탭 `new` 를 그대로 옮긴 **기본 탭 정의**. 아이템(그룹) 5 · 컬럼 22 · 자동 이동 6규칙 ·
맨 오른쪽 고정 열 «컨택 이동». D76 대로 «설치» 단계 없이 워크스페이스 생성 시 보장된다
(`ensureDefaultTabs`, 포트 `BoardsRepo` 경유 — 003 테이블 직접 SQL 없음).

**구조 팩(`structure-packs/seoul-*`)을 고치지 않았다.** 그쪽은 먼데이 실측 2026-08-05 의
아카이브고 `seoul-pack.test.ts` 가 컬럼 수를 `[24,21,24]` 로 못박은 채 마이그레이션 040 과
대조한다. 거기에 목업 v6 을 덮으면 실측 추적성이 사라지고 기존 마이그레이션을 고쳐야 한다
(AGENTS.md §9.1 금지). 그래서 «제품의 기본값» 을 별도 모듈로 세웠다 —
설계도 §3 «➕ 새로 필요한 것» 1번(워크스페이스 생성 시 기본 탭 시드)이 바로 이것이다.

### 목업 계약의 결함 1건 — 이동 규칙은 6개가 맞다

`extractMockupContract()` 는 탭 `new` 의 이동 규칙을 **10개**로 보고한다. 6개가 맞다.
목업에 이동 표가 두 벌 있다 — `MOVE.new`(낡은 판)와 `MOVE2.new`(컬럼별 판, null 두 칸을
채운 교정판). 둘 다 같은 «상담 상황» 규칙인데, 추출기가 `MOVE[key]` 를 맨 오른쪽 고정 열
(`PINR.new` = 컨택 이동)의 규칙으로 돌린다. `work`·`contact` 에서는 그 가정이 맞지만
`new` 에서만 어긋난다 — `MOVE.new` 의 값(1차 부재·2차 상담예약·보류·거절)은 컨택 이동의
선택지(컨택 대기·컨택 이동·거절)가 아니라 상담 상황의 값이다.
→ 그 4개는 컬럼이 잘못 붙은 중복이고, 넣어도 영원히 발화하지 않는 죽은 설정이 된다.
→ 사람이 읽는 `dump-mockup.mjs new` 출력이 `MOVE2` 만 써서 6개를 찍는 것과, 설계도 §1 표
  «상담 상황 기준 6규칙» 이 이를 뒷받침한다.
→ **NG-02(BBE-140) 인계**: `qa-app.mjs` 의 «자동 이동 규칙 차이 10개» 도 같은 4개 과다 계상.
→ 이 사실 자체를 `new-lead.test.ts` 에 «감시선» 테스트로 고정했다 — 추출기가 고쳐지면
  그 테스트가 깨지고, 그때 이 항목을 같이 지우면 된다.

### 곁가지로 드러난 실동작 결함 2건 (고쳤다)

- **`GroupTable` 이 `is_readonly` 를 안 봤다.** 서비스는 이미 그 칸의 쓰기를 전부 거부하는데
  (`boards/service.ts` «읽기 전용 칸») 표는 편집창을 열어 줬다. ƒ 수식 칸(D79)에도 해당한다.
  ✉ 발송 칸에서는 특히 위험하다 — 안전장치(BBE-148)가 붙는 순간 열려 있는 편집창이 곧
  돈이 나가는 통로가 된다. 화면과 서버가 같은 답을 하도록 한 줄 고쳤다.
- **`BoardToolbar` 의 필터 칩 목록에 `status` 타입이 빠져 있었다.** BBE-123 이 타입 15종을
  넣으며 select 에서 «상태» 를 분리했는데 이 목록은 같이 안 늘었다. 그래서 신규리드의 핵심
  필터인 「상담 상황」·「컨택 이동」·「피드백 상황」이 칩으로 아예 안 떴다(표에서는 잘 그리면서).

### 목업과 «의도적으로» 다른 5곳 — 전부 테스트에 근거를 등록했다

`부재 안내` 빈 라벨 → 값 없음(null, validation 이 빈 라벨 거부) · `담당자`·`협업자` 정적
선택지 없음(D71~D75, 멤버 계정에서 옴) · `시군구` 예시 12개 → 실측 지역 사전 222지에서 생성
(종속 선택은 BBE-127 소유) · `매출 구간`·`사업자 유형` 목업이 비워 둔 자리(후자는 002
`field_presets.biz_reg_type` 6종 재사용 — 국세 분류라 고객 고유값 아님).
근거가 등록되지 않은 차이가 생기면 `new-lead.test.ts` 가 실패한다.

### 측정 — qa-app 숫자가 왜 안 움직였나

`qa-app.mjs` 는 앱 쪽 정본으로 `structure-packs/seoul-pack.ts` 를 **하드코딩된 경로**로 읽는다.
이 카드의 산출물은 그 팩이 아니라 기본 탭이라 그 숫자에 잡히지 않는다. 팩을 고쳐 숫자를
내리는 것은 위 «구조 팩을 고치지 않았다» 이유로 불가하다.
→ **NG-02(BBE-140) 인계**: qa-app 가 기본 탭도 읽게 넓혀야 탭 `new` 의 66건이 실제로 줄어든다.
→ 그동안의 대체 측정으로 `new-lead.test.ts` 가 같은 계약 API(`extractMockupContract`)로
  목업과 직접 대조한다(19건).

### 화면 확인 — ❌ NOT_RUN (머지 전 해소 필요 · AGENTS.md §3)

1440px·375px 스크린샷을 **찍지 못했다.** 두 가지가 막는다.

1. **D24 권한 게이트가 hosted Supabase 를 요구한다.** `/boards/[id]` 는 `effective_permission`
   RPC 를 부르고, `lib/perm/server.ts` 가 조회 실패를 «불허» 로 수렴시킨다(의도된 fail-closed
   보안 경계 — 약화시키지 않았다. DG-04 영역 · 2단 검수 대상). env 없는 로컬에서는 항상 404.
2. **이 세션의 Browser pane 이 표시되지 않아** `computer screenshot` 이 «컴포지팅 없음» 으로
   타임아웃한다. BBE-103·BBE-142 가 보고한 것과 같은 재현성 있는 세션 환경 제약이다.

그 대신 기계로 잴 수 있는 관통 기준 전부를 `new-lead.render.test.tsx` 가 실제 렌더·실제 저장
경로로 고정한다(22컬럼 순서 · sticky 고정열 · 저장 후 재조회 · 6규칙 이동 · 칩 필터 ·
...
- 화면 스크린샷 1440/375 (위 사유 · **머지 전 해소 필요**)
- hosted DB 적용 — `source`·`move_rule_jsonb`·`is_readonly`·`right_pinned` 컬럼이 실제 DB 에
  없다(BBE-141 실측). DG-02 의 BBE-146 정합화 마이그레이션 대기. 지시대로 **로컬까지만** 했고
  `supabase/migrations/` 에 아무것도 넣지 않았다(그 디렉터리는 DG-02 소유).
