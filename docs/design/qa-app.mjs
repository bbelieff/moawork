...
//
// 39 → 48 (2026-08-20): 총괄 직접 지시로 «진행상항» 오탈자를 «진행상황» 으로 고쳤다.
// CLAUDE.md 「기준의 우선순위」 1번(총괄 직접 지시) > 2번(목업). 목업 v6 은 «진행상항» 을
// 자기 내부 조회 키로 ~10곳에서 쓰므로 목업은 고치지 않는다. 늘어난 9건은 전부 라벨
// 한 글자 차이의 파생이다 — 목업에만 1(진행상항) + 앱에만 1(진행상황) + 자동이동 8
// (같은 4규칙이 라벨 절반만 달라져 목업만/앱만으로 갈라진 것) − 선택지 1(라벨 불일치로
